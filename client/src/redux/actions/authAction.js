    ...
    try {
        ...
        if (response.data.success) {
            localStorage.setItem(
                GLOBALTYPES.LOCAL_STORAGE_REFRESH_TOKEN_NAME,
...
export const registerUser = async (userForm) => {
    try {
        const response = await axios.post(
            `${GLOBALTYPES.ApiUrl}/auth/register`,
            userForm
        );
        return response;
    } catch (error) {
...
export const loginUser = (userForm) => async (dispatch) => {
    try {
        const response = await axios.post(
            `${GLOBALTYPES.ApiUrl}/auth/login`,
            userForm
        );
        if (response.data.success) {
            localStorage.setItem(
                GLOBALTYPES.LOCAL_STORAGE_ACCESS_TOKEN_NAME,
...
export const refreshToken = () => async (dispatch) => {
    const res = await axios.put(`${GLOBALTYPES.ApiUrl}/auth/refreshToken`, {
        refreshToken:
            localStorage[GLOBALTYPES.LOCAL_STORAGE_REFRESH_TOKEN_NAME],
    });
    if (res.data.success) {
